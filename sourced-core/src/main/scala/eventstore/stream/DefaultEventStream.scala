...
import eventstore.metadata.{EventMetadata, StreamMetadata}
...
class DefaultEventStream(private val streamKey: StreamKey,
  ...
  extends EventStream with Logging{
  ...
  override def push(events: AnyRef*): Future[Try[Unit]] = {
  ...
  private[eventstore] def pushSync(msg: AnyRef): Unit = {
